...
:root{--f:#ff4500;--e:#ff8c00;--g:#ffd700;--bg:#08080f;--cr:#fff5e6;--dm:#55556a}
html,body{min-height:100%;background:var(--bg);color:var(--cr);font-family:'Syne',sans-serif}
body::before{content:'';position:fixed;inset:0;pointer-events:none;z-index:0;
  background:
    radial-gradient(ellipse 65% 45% at 12% 92%,rgba(255,50,0,.09) 0%,transparent 55%),
...
.pg{position:relative;z-index:1;max-width:700px;margin:0 auto;padding:48px 20px 96px}
.hdr{text-align:center;margin-bottom:44px}
.fl{display:block;font-size:64px;line-height:1;margin-bottom:4px;
  animation:sw 2s ease-in-out infinite alternate;transform-origin:50% 90%}
@keyframes sw{from{transform:rotate(-4deg) scale(.94)}to{transform:rotate(4deg) scale(1.06)}}
h1{font-family:'Bebas Neue',sans-serif;font-size:clamp(58px,13vw,100px);letter-spacing:6px;line-height:.86;
  background:linear-gradient(150deg,#fff9f0 0%,#ffb347 38%,#ff4500 68%,#a02800 100%);
...
.bdg{display:inline-flex;align-items:center;gap:7px;margin-top:16px;padding:5px 16px;
  background:rgba(255,69,0,.08);border:1px solid rgba(255,69,0,.2);border-radius:99px;
...
.ic{background:rgba(255,255,255,.028);border:1px solid rgba(255,69,0,.2);
...
.ii{flex:1;min-width:0;background:rgba(0,0,0,.45);border:1px solid rgba(255,255,255,.07);
  border-radius:11px;padding:13px 16px;font-family:'DM Mono',monospace;font-size:13px;
  color:var(--cr);outline:none;transition:border-color .2s,box-shadow .2s}
...
.ib{background:linear-gradient(135deg,#ff4500,#ff7400);border:none;border-radius:11px;
  padding:13px 22px;color:#fff;font-family:'Bebas Neue',sans-serif;font-size:19px;
  letter-spacing:2.5px;cursor:pointer;white-space:nowrap;
  box-shadow:0 4px 24px rgba(255,69,0,.28);transition:transform .15s,box-shadow .15s,opacity .15s}
...
.lfl{display:block;font-size:52px;margin-bottom:18px;
  animation:bn .65s ease-in-out infinite alternate}
@keyframes bn{from{transform:scale(.87) rotate(-6deg);filter:brightness(.85)}
  to{transform:scale(1.13) rotate(6deg);filter:brightness(1.25)}}
.lt{font-family:'Bebas Neue',sans-serif;font-size:26px;letter-spacing:4px;color:var(--e);margin-bottom:10px}
.ls{font-size:13px;color:var(--dm);line-height:1.8}
.sg{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px}
.sb{background:rgba(255,255,255,.025);border:1px solid rgba(255,255,255,.06);
...
.rc{background:rgba(255,50,0,.05);border:1px solid rgba(255,69,0,.28);
  border-radius:18px;padding:30px;margin-bottom:16px;position:relative;overflow:hidden}
.rc::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;
  background:linear-gradient(90deg,transparent,#ff4500 25%,#ffd700 55%,#ff8c00 80%,transparent)}
.rh{display:flex;align-items:center;gap:9px;margin-bottom:14px}
.rtl{font-family:'Bebas Neue',sans-serif;font-size:20px;letter-spacing:3px;color:var(--f)}
.vt{margin-left:auto;background:linear-gradient(135deg,#ff4500,#ff8c00);
  border-radius:99px;padding:4px 14px;font-size:10px;font-weight:800;letter-spacing:2px;text-transform:uppercase}
.ap{font-family:'DM Mono',monospace;font-size:11px;color:var(--dm);background:rgba(0,0,0,.3);
  border-radius:7px;padding:8px 11px;margin-bottom:18px;word-break:break-all;
...
.ab{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);
  border-radius:9px;padding:9px 16px;color:var(--cr);font-size:13px;font-weight:700;
  cursor:pointer;transition:all .2s;text-decoration:none;
  display:inline-flex;align-items:center;gap:7px}
.ab:hover{background:rgba(255,255,255,.08);border-color:rgba(255,255,255,.2)}
.eb{background:rgba(255,0,0,.06);border:1px solid rgba(255,80,80,.22);
  border-radius:13px;padding:20px 22px;color:#ff8080;font-size:14px;line-height:1.7}
.eb b{color:#ff5a5a;display:block;margin-bottom:8px;font-size:15px}
footer{text-align:center;margin-top:64px;padding-top:20px;
...
const fU = n => { const v=parseFloat(n||0); return v>=1e6?(v/1e6).toFixed(2)+'M':v>=1e3?(v/1e3).toFixed(1)+'K':v.toFixed(2) }
const fH = h => h ? h.slice(0,10)+'…'+h.slice(-6) : ''
const fD = ts => ts ? new Date(parseInt(ts)*1000).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'2-digit'}) : ''
...
export default function Home() {
  ...
  async function go() {
    ...
    if (!/^0x[0-9a-fA-F]{40}$/i.test(a)) {
      setErr('Invalid address — 42 characters starting with 0x')
      return
    }
    ...
    try {
      ...
      let json
      try { json = JSON.parse(text) } catch { throw new Error('Bad response: ' + text.slice(0,200)) }
      ...
      <main className="pg">
        ...
        <div className="ic">
          ...
          <div className="ir">
            ...
            <button className="ib" onClick={go} disabled={busy}>
              {busy ? '⏳ COOKING…' : '🔥 ROAST IT'}
            </button>
          </div>
        </div>

        {err && !busy && (
          <div className="eb"><b>⚠️ Error</b>{err}</div>
        )}

        {busy && (
          <div className="lb">
            <span className="lfl">🔥</span>
            <div className="lt">Cooking Your Roast…</div>
            <div className="ls">
              Fetching live Arc Testnet data server-side<br/>
              Writing your savage roast with Claude AI<br/>
              ~10–15 seconds
            </div>
          </div>
        )}
        ...
        {data && !busy && (<>
          ...
          <div className="rc">
            <div className="rh">
              <span>🔥</span>
              <span className="rtl">THE ROAST</span>
              <span className="vt">{v}</span>
            </div>
            <div className="ap">{data.address}</div>
            <div id="roast-text" className="rx">{data.roast}</div>
          ...
          <div className="ac">
            <button className="ab" onClick={() => {
              navigator.clipboard.writeText(document.getElementById('roast-text')?.innerText || '')
              setCopied(true); setTimeout(() => setCopied(false), 2000)
            }}>{copied ? '✅ Copied!' : '📋 Copy Roast'}</button>
            <a className="ab" href={`https://testnet.arcscan.app/address/${data.address}`} target="_blank" rel="noopener">
              🔍 View on Arcscan
            </a>
            <button className="ab" onClick={() => { setAddr(''); setData(null); setErr('') }}>
              🔄 Roast Another
            </button>
          </div>
        </>)}
        ...
        <footer>
          ...
          {' · '}Arc Testnet RPC{' · '}Claude AI<br/>
          🔥 Roasting fake wallets since 2025 · ngmi
        </footer>
      </main>
    </>
  )
